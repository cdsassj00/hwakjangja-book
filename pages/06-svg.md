...
**사원증 이백 장을 한 번에 만드는 도구**입니다.

도안 SVG 하나와 명단 CSV를 올리면 사람마다 사원증이 만들어져 봉투로 내려옵니다. 인쇄용 이미지도 함께 나옵니다.

총무팀 이 주임이 디자인 프로그램에서 도안을 열어 세 군데를 고치고 저장하기를 이백 번 반복하던 일이 몇 분으로 줄어듭니다. 그리고 다 만든 뒤에 부서명 하나가 바뀌어도 명단만 고쳐 다시 돌리면 됩니다.

같은 도구가 워크숍 명찰, 행사 참가증, 좌석표에도 그대로 쓰입니다. 도안만 바꾸면 됩니다.
...
**이름이 도안 밖으로 삐져나갑니다.** 긴 이름이나 부서명에서 생깁니다. 지시문에 "글자 수가 많으면 `font-size`를 줄여 안에 들어가게 해 줘"를 넣으십시오. **이백 장을 다 만든 뒤에 발견하면 처음부터 다시 해야 하니, 첫 세 장을 미리 보여 달라는 조건도 함께 넣습니다.**

**글꼴이 다른 컴퓨터에서 달라 보입니다.** SVG는 "이 글꼴로 그려라"라고 적어 둘 뿐이라 그 글꼴이 없는 컴퓨터에서는 다른 모양이 나옵니다. 인쇄소에 넘길 파일이라면 글자를 그림으로 굳혀야 하는데, 굳히면 더 이상 고칠 수 없으니 **고칠 일이 끝난 뒤 마지막에** 하십시오.

**색이 인쇄물에서 달라 보입니다.** 화면과 인쇄는 색을 만드는 방식이 다릅니다. 중요한 인쇄물이라면 시안을 한 장 뽑아 확인한 뒤 전량을 인쇄하십시오.

**만든 파일이 파워포인트에 안 붙습니다.** 옛 버전 오피스는 SVG를 받지 않습니다. 그럴 때는 PNG로도 함께 내려받게 조건을 더하면 됩니다.
