...
구조 이야기를 하기 전에 당장 급한 문제부터 풀고 가겠습니다. 지금 손에 `.hwpx` 파일이 있는데 열리지 않는다면 방법은 이렇습니다.

정식 프로그램은 한컴오피스의 **한글**입니다. 만들고 고치는 일을 제대로 하려면 이것이 필요하고 유료입니다. 다만 읽기만 하면 되는 경우가 훨씬 많지요. 그럴 때는 한컴이 무료로 배포하는 **한컴오피스 뷰어**를 설치하면 됩니다. 보는 것과 인쇄는 되고 편집만 막혀 있습니다. 컴퓨터에 무엇도 설치하고 싶지 않다면 네이버 웨일 브라우저나 일부 무료 오피스 프로그램이 열어 주기도 하는데, 이 경우 글꼴이나 표 모양이 원본과 조금 달라질 수 있습니다. 급하게 내용만 확인할 때 쓰시면 됩니다.

여기에 한 가지가 더 있습니다. 이 장에서 배울 방법을 쓰면 **프로그램을 하나도 설치하지 않고도 문서에 적힌 글자를 꺼낼 수 있습니다.** 서식까지 그대로 보는 것은 아니지만, 급히 내용을 확인하거나 다른 곳에 옮겨 적어야 할 때는 이 편이 빠릅니다. 그리고 이 방법을 알아야 뒤에 나올 자동화가 가능해집니다.
...
이 이야기가 공문 한 종류에만 해당하지 않는다는 점이 중요합니다.

**인사팀 최 주임의 1월입니다.** 연말정산 철이 되면 재직증명서를 떼 달라는 요청이 몰립니다. 직원 이백 명 남짓한 회사에서 한 달에 백오십 건쯤 나갑니다. 최 주임은 양식 파일을 열어 이름과 주민등록번호 앞자리와 입사일과 직위, 이렇게 네 군데를 고치고 다른 이름으로 저장합니다. 한 건에 이 분씩만 잡아도 다섯 시간입니다. 그런데 진짜 문제는 시간이 아닙니다. 서른 건쯤 지나면 손이 기계적으로 움직여서, 이름은 바꿨는데 입사일은 앞사람 것이 남은 증명서가 나갑니다. 그리고 그건 회사 밖으로 나가는 문서입니다.

**총무팀의 위촉장입니다.** 매년 3월 자문위원 마흔 명에게 위촉장을 발송합니다. 위원 이름과 소속과 임기만 다르고 나머지 문장은 토씨 하나까지 같습니다. 교육 담당자가 과정이 끝날 때마다 만드는 수료증 서른 장도, 영업팀이 거래처별로 찍어 내는 견적서 표지도 사정이 다르지 않습니다.

공통점은 하나입니다. **서식은 이미 완성되어 있고 바뀌는 것은 몇 개의 칸뿐이라는 것입니다.** 그리고 그 몇 개의 칸이 어디 있는지는 방금 상자를 열어 확인했습니다.

**그런데 만드는 것보다 중요한 일이 하나 더 있습니다.** 백 장을 만들었다면 백 장이 제대로 만들어졌는지 확인해야 합니다. 사람은 이 일을 못 합니다. 백 개의 파일을 하나씩 열어 이름과 날짜를 대조하려면 만드는 것보다 오래 걸리니까요. 그래서 대부분은 몇 장만 열어 보고 나머지는 믿고 보냅니다. 최 주임의 증명서에 앞사람 입사일이 남아 있던 사고가 그렇게 나갑니다.

구조를 알면 이것이 가장 쉬운 일이 됩니다. 만들어진 파일마다 상자를 열어 본문 글자를 꺼낸 뒤, 명단에 있는 이름이 정확히 한 번씩 들어 있는지 세어 보면 됩니다. 앞사람 이름이 남아 있다면 그 파일에는 이름이 두 개 들어 있을 테니 즉시 걸립니다. 발송 전에 "백 장 모두 이상 없음"을 확인하고 보내는 것과, 몇 장만 보고 믿고 보내는 것은 완전히 다른 일입니다.

같은 방법으로 **찾는 일**도 됩니다. 폴더에 쌓인 한글 문서 수백 개 중에서 특정 문구가 든 것만 골라내야 할 때, 한글 프로그램이 없어도 각 파일의 상자를 열어 글자만 훑으면 됩니다. 옛 사명이 남아 있는 문서를 찾거나, 계약서에서 특정 조항이 빠진 것을 골라내는 일이 여기 해당합니다.

그래서 여러분의 업무 중에서 이 방법이 통할 일을 찾는 기준도 자연스럽게 나옵니다. 지난달에 만든 문서를 이번 달에 다시 열어 몇 글자만 고쳐 다른 이름으로 저장하고 있다면, 그 문서가 바로 대상입니다. 한 번 도구를 만들어 두면 그다음부터는 명단만 갈아 끼우면 되므로, 매달 반복되는 일일수록 값어치가 큽니다.
